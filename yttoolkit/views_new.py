...
def start_download(request):
    """View to start a new YouTube MP3 download (for web interface)"""
    if request.method == "POST":
        ...
        try:
            ...
            elif youtube_mp3.download_status in ["completed", "in_progress"]:
                messages.warning(
                    request,
...
def download_file(request, download_id):
    ...
    if not youtube_mp3.is_downloadable:
        raise Http404("File not found or not ready for download")
...
# Backward compatibility: Keep the old API endpoints
class LegacyDownloadStatusAPIView(APIView):
    """
    ...
    @extend_schema(
        summary="Get download status (Legacy)",
        description="Get download status - legacy endpoint for backward compatibility",
        responses={200: YouTubeMP3Serializer},
    )
    def get(self, request, download_id):
        ...
        try:
            youtube_mp3 = get_object_or_404(YouTubeMP3, id=download_id)
            serializer = YouTubeMP3Serializer(youtube_mp3)
...
def api_start_download(request):
    """Legacy API endpoint to start a YouTube MP3 download"""
    try:
...
def api_download_status(request, download_id):
    """Legacy API endpoint to check download status"""
    try:
        youtube_mp3 = get_object_or_404(YouTubeMP3, id=download_id)
